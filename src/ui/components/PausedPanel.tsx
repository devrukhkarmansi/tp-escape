...
type Props = {
  timeLeft: number
  /** Solo: always. Crew: only the host can resume. */
  canResume: boolean
  /** Crew only: who's holding the countdown, for everyone else. */
  hostName?: string
  onResume: () => void
  onExit: () => void
  exitLabel: string
  ...
  return (
    <main className="mx-auto flex min-h-[70dvh] w-full max-w-md flex-col items-center justify-center gap-4 px-5 text-center">
      <p className="font-display text-[11px] tracking-[0.18em] text-ink-muted uppercase">
        {hostName ? `Paused by ${hostName}` : 'Mission paused'}
      </p>
      ...
      <p className="max-w-xs text-sm/6 text-balance text-ink-muted">
        HALCYON is holding the countdown. The puzzles stay hidden until{' '}
        {canResume ? 'you resume' : 'the host resumes'}.
      </p>
      {canResume && (
        <button
          type="button"
