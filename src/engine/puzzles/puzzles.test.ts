...
import { anomaly, scopeLayout } from './anomaly.ts'
...
describe('split pieces', () => {
  const splittable = [caesar, glyph, morse, gauge, wiring, anomaly]
...
describe('anomaly', () => {
  ...
  it('answers with the digits the rounds read back', () => {
    expect(
      failing(
        puzzles,
        (p) =>
          scopeOf(p)
            .scope.map((round) => round.digit)
            .join('') === p.answer,
      ),
    ).toEqual([])
  })

  it('has exactly one contact matching both halves of the report, every round', () => {
    expect(
      failing(puzzles, (p) => {
        const { scope, signature } = scopeOf(p)
        return scope.every((round) => {
          const matching = round.contacts.filter((contact) =>
            signature!.traits.every((trait) => contact[trait] === odd[trait]),
          )
          return matching.length === 1 && round.contacts[round.faultIndex] === matching[0]
        })
      }),
    ).toEqual([])
  })

  it('puts decoys on the scope, so the odd-looking one is never simply the answer', () => {
    expect(
      failing(puzzles, (p) => {
        const { scope, signature } = scopeOf(p)
        return scope.every((round) =>
          signature!.traits.every(
            (trait) =>
              round.contacts.filter((contact) => contact[trait] === odd[trait]).length >= 2,
          ),
        )
      }),
    ).toEqual([])
  ...
  it('reads the report in words the crew can say out loud', () => {
    expect(
      failing(puzzles, (p) =>
        /^(amber|small|steady \(not pulsing\)) and /.test(scopeOf(p).signature!.reads),
      ),
    ).toEqual([])
  })

  it('splits into the scope and the report, with the signature only on the report', () => {
    expect(
      failing(puzzles, ({ split }) => {
        const [scope, report] = split!.pieces
        return (
          scope!.visual?.type === 'anomaly' &&
          scope!.visual.signature === undefined &&
          !!report!.text?.includes('Fault signature')
        )
      }),
    ).toEqual([])
  })

  it('grows from two rounds to three, on a busier scope', () => {
    const shape = (level: number) => everyPuzzle(anomaly, [level]).map(scopeOf)
    expect(new Set(shape(0).map((v) => v.scope.length))).toEqual(new Set([2]))
    expect(new Set(shape(1).map((v) => v.scope.length))).toEqual(new Set([3]))
    expect(Math.max(...shape(0).map((v) => v.scope[0]!.contacts.length))).toBeLessThan(
      Math.min(...shape(1).map((v) => v.scope[0]!.contacts.length)),
    )
  })
})

describe('the anomaly scope layout', () => {
  it('lays out the same scope on every phone, so a crew can point at it', () => {
    expect(scopeLayout('4', 1, 20)).toEqual(scopeLayout('4', 1, 20))
  })

  it('keeps every contact inside the scope', () => {
    for (const round of [0, 1, 2]) {
      for (const spot of scopeLayout('7', round, 24)) {
