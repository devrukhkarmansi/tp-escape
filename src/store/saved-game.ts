import { isPaused, type GameState } from '../engine/game.ts'
...
export function loadGame(storage: KeyValueStorage | undefined, now: number): GameState | null {
  try {
    ...
    // A paused game's clock is frozen, so it's still playable however long ago it was paused.
    if (!isPaused(state) && now >= state.endsAt) return null
    return state
  } catch {
    return null
  }
