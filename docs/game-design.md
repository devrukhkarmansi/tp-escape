...
| Length                         | Quick Run (5 systems, 6 min) · Full Shift (8, 10 min) · Deep Space (12, 16 min)                                                                                 | Halved after the first playtest (2026-09-11): about 75 s per system in every mode            |
...
| Pause                          | Solo: pause any time. The clock stops, puzzles are hidden, and the paused time is added back on resume. Multiplayer: decided in stage 3                         | Real life interrupts; hiding the puzzles stops pause being free thinking time                |
| Final minute                   | A tick every second (higher in the last 10), a red vignette pulsing once a second, a HALCYON warning, and an alarm at 0:00. Mute button, remembered             | The "we have one minute!" moment. One pulse a second stays well within seizure-safety limits |
...
| Split puzzles (4D, 2026-09-15) | About 1 system in 3 is split in crew games; pieces are shown on screen only (no private documents); a dropped player's pieces move to someone still connected   | See [puzzle-system.md](puzzle-system.md#split-puzzles)                                       |
...
> **Changed in stage 4D:** there is no `intel` collection. Every phone works out who holds each piece of a split puzzle from the game seed and the player list, and shows only its own. A private document per player would cost extra writes and still wouldn't stop peeking, because answers already come from the seed. See [puzzle-system.md](puzzle-system.md#split-puzzles).
